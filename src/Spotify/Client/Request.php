...
class Request
{
    ...
    protected function handleResponseError($body, $status)
    {
        ...
        if (isset($error->message, $error->status)) {
            ...
            throw new ApiAuthException($parsedBody->error_description, $status);
        } elseif ($body) {
            ...
            throw new ApiException('An unknown error occurred.', $status);
        }
    }
    ...
    public function send($method, $url, $parameters = [], $headers = [])
    {
        ...
        if (curl_error($ch)) {
            ...
            throw new ApiException('cURL transport error: ' . $errno . ' ' . $error);
        }
